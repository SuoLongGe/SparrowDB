...
public class test_shard_path_fix {
    public static void main(String[] args) {
        System.out.println("=== 测试分片文件路径修复 ===");
        ...
        try {
            ...
            // 2. 先删除可能存在的分片
            System.out.println("\n删除现有分片...");
            ExecutionResult dropResult = engine.executeSQL("DROP SHARD demo_products");
            System.out.println("删除分片结果: " + dropResult.isSuccess());
            
            // 3. 创建新的分片
            System.out.println("\n创建新分片...");
            ExecutionResult createResult = engine.executeSQL("CREATE SHARD demo_products BY supplier USING RANGE (3)");
            ...
            if (createResult.isSuccess()) {
                ...
                ExecutionResult insertResult = engine.executeSQL(
                    "INSERT INTO demo_products (product_id, product_name, category, price, stock_quantity, supplier, created_date) " +
                    "VALUES (999, 'Path Test Product', 'Test', 99.99, 1, 'TestSupplier', '2024-01-01')"
                );
